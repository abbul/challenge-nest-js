...
import { BodySearchUsers, SearchUsers } from 'core/user/architecture/search-users';
...
export class AppController {
  ...
  async getUsersController(@Query() query: SearchCommon, @Res({ passthrough: true }) res: Response): Promise<any> {
    const cu = new GetUsers(new UserApiGithub())
    const result = await cu.execute(query);

    const { httpAdapter } = this.httpAdapterHost
    httpAdapter.setHeader(res, 'Link', result.links.map((link) => `<${link.href}>; rel="${link.rel}"`).join(', '))
